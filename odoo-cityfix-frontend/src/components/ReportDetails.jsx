...
const reportData = {
  ...
  description: 'The main road in C.G. Road, Ahmedabad, is riddled with potholes, making it dangerous and difficult to travel on. The issue is particularly bad near the main intersection, causing traffic to slow down significantly.',
  location: {
...
const ActivityItem = ({ item, isLast }) => (
    <div className="relative pl-8">
        {!isLast && <div className="absolute left-3 top-3 -bottom-3 w-0.5 bg-gray-200"></div>}
        ...
        <p className="text-sm text-gray-500">{item.timestamp} - by {item.actor}</p>
    </div>
);
...
function ReportDetailsPage() {
  const handleReportSpam = () => {
    ...
    alert("This issue has been reported as spam. An admin will review it shortly.");
  };
      ...
      <div className="container mx-auto max-w-6xl p-4 sm:p-6 lg:p-8">
        ...
        <main className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="relative h-64 md:h-80 w-full">
            <img src={reportData.imageUrl} alt={reportData.title} className="w-full h-full object-cover"/>
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
            <div className="absolute bottom-0 left-0 p-6 md:p-8">
              <h1 className="text-3xl md:text-4xl font-bold text-white leading-tight">{reportData.title}</h1>
            </div>
          </div>
            ...
            <div className="lg:col-span-2 space-y-8">
              <div>
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mb-4 text-sm text-gray-500">
                  <span className="font-semibold text-teal-600 bg-teal-50 px-3 py-1 rounded-md">{reportData.category}</span>
                  <span>|</span>
              ...
              <div>
                ...
                <div className="space-y-6">
                    {reportData.activity.map((item, index) => (
                        <ActivityItem key={index} item={item} isLast={index === reportData.activity.length - 1}/>
                    ))}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
                <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                    ...
                    <div className="flex items-center gap-3 text-sm text-gray-600">
                        ...
                        <span>Reported by: <span className="font-semibold">{reportData.reportedBy}</span></span>
                    </div>
                </div>
                ...
                <div className="bg-gray-50 p-5 rounded-lg border border-gray-200">
                    ...
                    <div className="flex items-start gap-3 text-sm text-gray-600 mb-4">
                        ...
                        <span className="font-semibold">{reportData.location.address}</span>
                    </div>
                    {/* --- GOOGLE MAP INTEGRATION --- */}
                    <div className="h-48 rounded-md overflow-hidden border border-gray-200">
                        <MapComponent lat={reportData.location.lat} lng={reportData.location.lng} />
                    </div>
                </div>
